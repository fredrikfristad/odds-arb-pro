...
const explainParlay = (parlay) => {
  ...
  const marketTypes = [...new Set(parlay.legs.map((leg) => leg.marketType))].join(", ");
  return [
    ...
    `Markedene i parlayen er: ${marketTypes}.`,
...
const hasNon1x2Leg = (legs) => legs.some((leg) => leg.marketType !== "1x2");
const oneMatchWinnerPerParlay = (legs) => legs.filter((leg) => leg.marketType === "1x2").length <= 1;
...
export const generateParlays = (matches, userOptions = {}) => {
  ...
  const candidateOptions = {
    norskTippingOnly: options.norskTippingOnly,
    exclude1x2: options.exclude1x2,
    includePlayerProps: options.includePlayerProps,
    allowedMarketTypes: options.allowedMarketTypes,
  };
  const allCandidates = buildCandidateBets(matches, historicalDataset, candidateOptions).filter((bet) => {
    ...
    if (bet.expectedValue < options.minExpectedValue) return false;
    return true;
  });
  ...
  for (const legs of attempts) {
    ...
    if (!oneMatchWinnerPerParlay(legs) && !typeConfig.allowSameGame) continue;
    if (options.requireNon1x2 && options.legCount > 1 && !hasNon1x2Leg(legs)) continue;
...
export const PARLAY_MARKET_OPTIONS = [
  { id: "1x2", label: "1X2" },
  { id: "double_chance", label: "Dobbelsjanse" },
  { id: "over_under", label: "Over/Under" },
  { id: "btts", label: "Begge lag scorer" },
  { id: "asian_handicap", label: "Asian handicap" },
  { id: "spread", label: "Handicap" },
  { id: "team_total_goals", label: "Lag totalt mål" },
  { id: "corners", label: "Cornere" },
  { id: "cards", label: "Kort" },
  { id: "clean_sheet", label: "Clean sheet" },
  { id: "win_totals", label: "Seier + over/under" },
  { id: "win_btts", label: "Seier + BTTS" },
  { id: "correct_score", label: "Korrekt resultat" },
  { id: "player_scorer", label: "Spiller scorer" },
  { id: "shots_on_target", label: "Skudd på mål" },
  { id: "first_goalscorer", label: "Første målscorer" },
];
