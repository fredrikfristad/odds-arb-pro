...
import { generateParlays, PARLAY_TYPE_OPTIONS, PARLAY_MARKET_OPTIONS } from "./parlay/parlayGenerator.js";
...
const WorldCupScreen = ({ matches, loading, source, error, quota, updatedAt, onRefresh, onSelect }) => {
  ...
  return (
    <div>
      ...
      <ParlayGeneratorScreen
        matches={worldCupMatches}
        title="VM 2026 Parlay Generator"
        description="Lag varierte VM-parlays basert på value, markedstype, risiko og eventuelt kun Norsk Tipping-odds."
        defaultWorldCupOnly
      />
    ...
    <Card style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 12, marginBottom: 12 }}>
        <div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 6 }}>
            ...
            {parlay.legs.every((leg) => leg.isNorskTipping) && <Badge color={C.green} bg={C.greenDim}>Norsk Tipping</Badge>}
          ...
          <div key={leg.id} style={{
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 5 }}>
                ...
                {leg.expectedValue >= 0.15 && <Badge color={C.green} bg={C.greenDim}>Høy EV</Badge>}
                {leg.statisticalSupport && <Badge color={leg.statisticalSupport === "High" ? C.green : leg.statisticalSupport === "Medium" ? C.amber : C.muted} bg={C.surfaceUp}>Statistisk støtte: {leg.statisticalSupport}</Badge>}
                {leg.isNorskTipping && <Badge color={C.green} bg={C.greenDim}>Norsk Tipping</Badge>}
              </div>
              ...
              {leg.reason && (
                <div style={{ color: C.muted, fontSize: 11, lineHeight: 1.4, marginTop: 5 }}>
                  {leg.reason}
                </div>
              )}
            </div>
      ...
      {parlay.score.correlationRisk > 0.25 && (
        <div style={{ marginTop: 10, color: C.amber, background: C.amberDim, border: `1px solid ${C.amber}44`, borderRadius: 8, padding: 10, fontSize: 12 }}>
          Høy korrelasjon mellom disse valgene. Korrelasjonsrisiko: {(parlay.score.correlationRisk * 100).toFixed(1)}%.
        </div>
      )}
...
const ParlayGeneratorScreen = ({ matches, title = "Parlay Generator", description, defaultWorldCupOnly = false }) => {
  const [settings, setSettings] = useState({
    ...
    targetTotalOdds: 10,
    riskLevel: "Medium",
    worldCupOnly: defaultWorldCupOnly,
    valueOnly: true,
    excludeSameGame: true,
    maxLegsPerMatch: 1,
    maxCorrelationRisk: 0.35,
    minExpectedValue: 0,
    norskTippingOnly: false,
    exclude1x2: false,
  ...
  const availableMarketTypes = useMemo(() => new Set(matches.flatMap((match) => match.markets || []).map((market) => market.type)), [matches]);
  const hasNorskTippingOdds = useMemo(() => matches.some((match) => match.markets?.some((market) => market.outcomes?.some((outcome) => outcome.odds?.some((odd) => {
    const key = `${odd.bookmaker || ""} ${odd.bookmakerName || ""}`.toLowerCase();
    return key.includes("norsk") || key.includes("norsktipping");
  })))), [matches]);
  ...
  return (
    <div>
      <Card style={{
        ...
        <h1 style={{ margin: "0 0 6px", fontSize: 28, letterSpacing: "-.04em" }}>{title}</h1>
        <p style={{ margin: 0, color: C.muted, lineHeight: 1.55, fontSize: 14 }}>
          {description || "AI-drevet modul som vurderer single bets først, filtrerer på positiv EV, kontrollerer korrelasjon og simulerer realistiske parleys basert på live odds og modellklare historikkfelter."}
        </p>
      </Card>
        ...
        <Card>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
            {[
              ...
              ["Norsk Tipping", { norskTippingOnly: true, valueOnly: false, riskLevel: "Medium", legCount: 3, minTotalOdds: 2, targetTotalOdds: 8, maxTotalOdds: 25 }],
            ].map(([label, preset]) => (
          ...
          {showAdvanced && (
            <div style={{
              ...
              {[
                ...
                ["norskTippingOnly", "Kun Norsk Tipping"],
                ["excludeSameGame", "Ekskluder same-game"],
                ["exclude1x2", "Ekskluder 1X2"],
                ["includePlayerProps", "Inkluder spillerprops"],
              ].map(([key, label]) => (
                <ControlTile key={key}>
              ...
              <ControlTile>
                <NumericInput value={Math.round(settings.minExpectedValue * 100)} onCommit={(value) => update("minExpectedValue", value / 100)} min={-100} max={300} label="Minimum EV (%)" help="0 betyr bare positiv EV. Sett lavere hvis du vil ha flere forslag." />
              </ControlTile>
            </div>
          )}
          ...
          <div style={{ marginTop: 12 }}>
            <SettingHint title="Markeder">Støtter 1X2, dobbelsjanse, over/under, BTTS, handicap, cornere, kort, lagmål, clean sheet og kombimarkeder når oddsfeeden har dem. Korrekt resultat og spillerprops utelates hvis datagrunnlaget er for svakt.</SettingHint>
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 12 }}>
            {PARLAY_MARKET_OPTIONS.map((market) => {
              const active = !settings.allowedMarketTypes.length || settings.allowedMarketTypes.includes(market.id);
              const available = availableMarketTypes.has(market.id);
              return (
                <button key={market.id} onClick={() => setSettings((current) => {
                  const currentList = current.allowedMarketTypes.length ? current.allowedMarketTypes : PARLAY_MARKET_OPTIONS.map((item) => item.id);
                  const next = currentList.includes(market.id)
                    ? currentList.filter((id) => id !== market.id)
                    : [...currentList, market.id];
                  return { ...current, allowedMarketTypes: next.length === PARLAY_MARKET_OPTIONS.length ? [] : next };
                })} style={{
                  ...buttonStyle,
                  padding: "6px 8px",
                  background: active ? C.accentDim : "transparent",
                  color: available ? (active ? C.accent : C.muted) : C.dim,
                  borderColor: active ? `${C.accent}66` : C.border,
                }}>
                  {market.label}{available ? "" : " (mangler)"}
                </button>
              );
            })}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
            ...
            {settings.norskTippingOnly && <Badge color={hasNorskTippingOdds ? C.green : C.amber} bg={hasNorskTippingOdds ? C.greenDim : C.amberDim}>{hasNorskTippingOdds ? "Norsk Tipping aktiv" : "Ingen Norsk Tipping-odds funnet"}</Badge>}
      ...
      <Section title="Genererte parlays" right={<button onClick={() => setNonce((value) => value + 1)} style={buttonStyle}>Generate New Parlay</button>}>
        {!upcomingMatches.length ? (
          ...
          <EmptyState
            ...
            text={settings.norskTippingOnly
              ? "Ikke nok Norsk Tipping-odds tilgjengelig for valgt parlay."
              : "Ingen spill matcher valgte innstillinger. Prøv å redusere antall legs, slå av Kun value bets, tillat flere markeder eller velg et preset."}
          />
        )}
      </Section>
    </div>
