      ...
      <Card style={{ marginBottom: 10 }}>
        <div className="wc-filter-grid">
          <input
            ...
            aria-label="Total innsats"
          />
          <select value={minMargin} onChange={(event) => setMinMargin(Number(event.target.value))} style={selectStyle}>
        ...
        {parlay.legs.map((leg) => (
          <div key={leg.id} style={{
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ color: C.dim, fontSize: 10, marginTop: 2 }}>
                Modell {(leg.modelProbability * 100).toFixed(1)}% · implied {(leg.impliedProbability * 100).toFixed(1)}% · EV {(leg.expectedValue * 100).toFixed(1)}%
              </div>
            </div>
  ...
  return (
    <div>
      ...
      <Section title="Generator-innstillinger">
        <Card>
          <div className="wc-filter-grid">
            <select value={settings.parlayType} onChange={(event) => update("parlayType", event.target.value)} style={selectStyle}>
              {PARLAY_TYPE_OPTIONS.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}
            </select>
            <input type="number" min="2" max="8" value={settings.legCount} onChange={(event) => update("legCount", Number(event.target.value))} style={selectStyle} aria-label="Antall legs" />
            <input type="number" min="1" value={settings.minTotalOdds} onChange={(event) => update("minTotalOdds", Number(event.target.value))} style={selectStyle} aria-label="Min total odds" />
            <input type="number" min="1" value={settings.maxTotalOdds} onChange={(event) => update("maxTotalOdds", Number(event.target.value))} style={selectStyle} aria-label="Max total odds" />
            <select value={settings.maxCorrelationRisk} onChange={(event) => update("maxCorrelationRisk", Number(event.target.value))} style={selectStyle}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10 }}>
            ...
            <select value={settings.maxLegsPerMatch} onChange={(event) => update("maxLegsPerMatch", Number(event.target.value))} style={selectStyle}>
      ...
      <Section title="Genererte parlays" right={<button onClick={() => setNonce((value) => value + 1)} style={buttonStyle}>Generate New Parlay</button>}>
        {parlays.length ? parlays.slice(0, 5).map((parlay) => (
          ...
          <EmptyState
            ...
            text="Det finnes ikke nok legs som matcher kravene akkurat nå. Senk min odds, øk max odds, skru av value-only, eller tillat flere legs fra samme kamp."
          />
        )}
      </Section>
    </div>
