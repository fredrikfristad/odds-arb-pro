      ...
      <Section title="Generator-innstillinger">
        <Card>
          <div style={{
            ...
            <ControlTile>
              <SelectField
                value={settings.parlayType}
                onChange={(event) => update("parlayType", event.target.value)}
                label="Parlay-type"
                help="Velger strategi: tryggere forslag, balansert risiko, høyere risiko eller AI value."
              >
            ...
            <ControlTile>
              <NumericInput
                ...
                integer
                label="Antall legs"
                help="Hvor mange enkeltspill som kombineres i parlayen."
              />
            </ControlTile>
            <ControlTile>
              <NumericInput
                value={settings.minTotalOdds}
                onCommit={(value) => update("minTotalOdds", value)}
                min={1}
                label="Min total odds"
                help="Laveste totalodds generatoren kan foreslå."
              />
            </ControlTile>
            <ControlTile>
              <NumericInput
                value={settings.targetTotalOdds}
                onCommit={(value) => update("targetTotalOdds", value)}
                min={1}
                label="Mål totalodds"
                help="Dette er oddsen generatoren prøver å treffe, for eksempel 100."
              />
            </ControlTile>
            <ControlTile>
              <NumericInput
                value={settings.maxTotalOdds}
                onCommit={(value) => update("maxTotalOdds", value)}
                min={1}
                label="Max total odds"
                help="Høyeste totalodds generatoren kan foreslå. Øk denne hvis du får få forslag."
              />
            </ControlTile>
            <ControlTile>
              <SelectField
                ...
                label="Risikonivå"
                help="Lav risiko prioriterer sannsynlighet. Høy risiko prioriterer å treffe høyere totalodds."
              >
            ...
            <ControlTile>
              <SelectField
                value={settings.maxCorrelationRisk}
                onChange={(event) => update("maxCorrelationRisk", Number(event.target.value))}
                label="Korrelasjonsgrense"
                help="Hvor strengt appen skal unngå bets som påvirker hverandre."
              >
                <option value={0.2}>Lav korrelasjon</option>
                <option value={0.35}>Medium korrelasjon</option>
                <option value={0.55}>Høy korrelasjon</option>
              </SelectField>
            </ControlTile>
          </div>
          <div style={{
            ...
            {[
              ["worldCupOnly", "Kun VM 2026", "Bruker bare kamper markert som VM 2026."],
              ["valueOnly", "Kun value bets", "Bruker bare bets med positiv modellert EV."],
              ["excludeSameGame", "Ekskluder same-game", "Hindrer flere legs fra samme kamp."],
            ].map(([key, label, help]) => (
              <ControlTile key={key}>
                ...
                <div style={{ color: C.muted, fontSize: 11, lineHeight: 1.35, marginTop: 8 }}>{help}</div>
              </ControlTile>
            ))}
            <ControlTile>
              <SelectField
                value={settings.maxLegsPerMatch}
                onChange={(event) => update("maxLegsPerMatch", Number(event.target.value))}
                label="Legs per kamp"
                help="Begrenser hvor mange valg generatoren kan ta fra samme kamp."
              >
                <option value={1}>Maks 1 leg per kamp</option>
                <option value={2}>Maks 2 legs per kamp</option>
                <option value={4}>Maks 4 legs per kamp</option>
              </SelectField>
            </ControlTile>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 8, marginTop: 12 }}>
            <SettingHint title="Kun kommende kamper">Generatoren filtrerer bort live, ferdige, kansellerte og utsatte kamper, og bruker ikke kamper som har startet.</SettingHint>
            <SettingHint title="Kun value bets">Når dette er aktivt brukes bare spill der modellert sannsynlighet er høyere enn bookmakerens implied probability.</SettingHint>
            <SettingHint title="Ekskluder same-game">Hindrer at flere legs fra samme kamp kombineres ukritisk.</SettingHint>
            <SettingHint title="Mål totalodds">Generatoren prøver å komme nær denne oddsen. Sett 100 hvis du vil ha en kombinasjon rundt 100 i odds.</SettingHint>
            <SettingHint title="Oddsgrenser">Min og max er harde grenser. Mål totalodds er ønsket nivå innenfor grensene.</SettingHint>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
